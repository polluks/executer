...
#ifdef __GNUC__
#ifndef __reg
# define __reg(a,b) register b asm(#a)
#endif
#endif
...
#define BEGINMTABLE2(name) static ULONG name##_dispatch(void); \
	struct EmulLibEntry GATE_##name##_dispatch = \
	{ \
...
#define BEGINMTABLE2(name) static ULONG __asm __saveds name##_dispatch(register __a0 struct IClass *cl, register __a2 Object *obj, register __a1 Msg msg){switch(msg->MethodID){
#endif /* __GNUC__ */

/*
 * Some common methods
 */
#define DECNEW(x) case OM_NEW:return(handleOM_NEW_##x(cl, obj, (APTR)msg));
#define DECCONST DECNEW // obsolete
#define DECDISPOSE(x) case OM_DISPOSE:return(handleOM_DISPOSE_##x(cl, obj, (APTR)msg));
#define DECDISP DECDISPOSE // obsolete
#define DECSET(x) case OM_SET:return(handleOM_SET_##x(cl, obj, (APTR)msg));
#define DECGET(x) case OM_GET:return(handleOM_GET_##x(cl, obj, (APTR)msg));
 ...
 */
#define DEFNEW(x) METHOD_INLINE static ULONG handleOM_NEW_##x(struct IClass *cl,Object*obj,struct opSet *msg)
#define DEFCONST DEFNEW

/*
 * OM_SET method
 */
#define DEFSET(x) METHOD_INLINE static ULONG handleOM_SET_##x(struct IClass *cl,Object*obj,struct opSet *msg)

/* 
 * OM_GET method
 */
#define DEFGET(x) METHOD_INLINE static ULONG handleOM_GET_##x(struct IClass *cl,Object*obj,struct opGet *msg)
 ...
 */
#define DEFDISPOSE(x) METHOD_INLINE static ULONG handleOM_DISPOSE_##x( struct IClass *cl,Object*obj,struct opSet *msg)
 ...
 */
#define DECSUBCLASS_NC(super,name,data) static struct MUI_CustomClass *mcc##name; \
	...
	{ \
		if (!(mcc##name = (struct MUI_CustomClass *)MUI_CreateCustomClass(NULL, super, NULL, sizeof(struct data), (APTR)DISPATCHERREF2(name)))) \
			...
			if (MUIMasterBase->lib_Version >= 20) \
				mcc##name->mcc_Class->cl_ID = (UBYTE *) #name; \
		return (TRUE); \
	} \
...
#define MUI_HOOK(n, y, z) \
	...
	static LONG n##_GATE2(struct Hook *h, y, z)
#else
#define DEFHOOKFUNC(r,n,z) static r n##_func(__reg(a1, z))
/*
 * r - return type
 * n - name 
 * y - a2
 * z - a1
 */
#define DEFHOOKFUNC2(r,n,y,z) static r n##_func(__reg(a2, y), __reg(a1, z))
#define DEFHOOK(n) static struct Hook n##_hook={{0,0},(HOOKFUNC)n##_func,0,0}

#define MUI_HOOK(n, y, z) \
	static LONG __asm __saveds n##_func(__reg(a0, struct Hook *h), __reg(a2, y), __reg(a1, z)); \
